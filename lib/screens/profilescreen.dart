...
import 'package:insta/widgets/followbutton.dart';
...
followuser() async
{
  ...
  final String otherid=widget.uid;
 final snaps=await FirebaseFirestore.instance.collection('users').doc(myid).get();

    final followinglist= snaps.get('following') as List<dynamic>;

    if(followinglist.contains(otherid))
    {
     await FirebaseFirestore.instance.collection('users').doc(myid).update(
        {'following': FieldValue.arrayRemove([otherid])
        });

            await FirebaseFirestore.instance.collection('users').doc(otherid).update(
                                  ...
                                  children: [
                                    FirebaseAuth.instance.currentUser!.uid ==
                                        ...
                                        ? FollowButton(
                                            text: 'Sign Out',
                                            backgroundColor:
                                            width>=websize ?MyColors.webBackgroundColor
